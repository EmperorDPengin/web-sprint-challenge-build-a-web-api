...
1. The core features of Node.js and Express and why they are useful.
  ...
  Node.js gave the ability to JS to be used outside the browser mode easily by allowing it to be able to write programs only prossible before in things like C#, or Python.

  Express is a framework that sits atop Node.J, making it wasier to create web applications and services. It can serve static content like HTML files as well as Building web applications and Serving Single Page Aplications (SPAs)


2. Understand and explain the use of Middleware.

  Middleware is a maner of keeping things tidy while setting up a server and data. It helps specific and reusable code to be placed in a separate section which is then addressed and checked regularly in order to help with main processes like checking user auth or if a form is filled properly while maintaning the main code clean and easy to read.

  It also helps modulate the project, because if a middleware fails it can easily be tracked.

3. The basic principles of the REST architectural style.

  Everything is a resource
  Each resource is accessible via an unique URL
...
4. Understand and explain the use of Express Routers.
  ...
  Using routers helps clean the server script by allocation code in specific files which then manage how the traffic to that route is handled.

  this allows for users to head to different parts of the webpage and to direct to specific data on a database.

5. Describe tooling used to manually test the correctness of an API.

  Postman? is that what this means? Postman and HTTpie?

  Postman is a kinda cumbersome application that needs to run in a separate window to testin manually has u switching back and forth but the response is more ligible if youa rent used to the terminal & it can be easier to send requests due to its simple UI but HTTPie is on terminal and can be ran while coding so it becomes a quicker process when testing for small things example if you coded a route properly

  while postman can be used for more complex things like checking cookies and reading and entire heading or something. so HTTPie and postman can be used together and neither is better. 
